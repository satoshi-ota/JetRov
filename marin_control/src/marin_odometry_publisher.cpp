...
public:
  ...
  void OdomPubCB (const marin_msgs::MarinStatus& input_msg)
  {
    current_time = ros::Time::now();
    dt = (current_time - last_time).toSec();

    omega = input_msg.angular_velocity.z;

    computeVelosity(input_msg.pulse_count);
    computeOdom();
  ...
  void computeOdom()
  {
    ...
    else
    {
      ...
      delta_x = r * (sin(theta + delta_theta)-sin(theta));
      delta_y = r * (cos(theta) - cos(theta + delta_theta));
    }
